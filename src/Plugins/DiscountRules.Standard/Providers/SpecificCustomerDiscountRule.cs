...
public class SpecificCustomerDiscountRule : IDiscountRule
{
    /// <summary>
    ...
    public async Task<DiscountRuleValidationResult> CheckRequirement(DiscountRuleValidationRequest request)
    {
        ...
        if (request.DiscountRule.Metadata != request.Customer.Id)
    ...
    {
        //configured 
        var result = "Admin/SpecificCustomer/Configure/?discountId=" + discountId;
        if (!string.IsNullOrEmpty(discountRequirementId))
            result += $"&discountRequirementId={discountRequirementId}";
        return result;
    }

    public string FriendlyName => "Must be specific customer";

    public string SystemName => "DiscountRules.Standard.MustBeSpecificCustomer";
}